module FoodCritic
  ...
  module CommandHelpers
    ...
    # @option options [Symbol] :file_type Alternative to specifying file name. One of: :attributes, :definition, :metadata, :provider, :resource
    def expect_warning(code, options={})
      if options.has_key?(:file_type)
        options[:file] = {:attributes => 'attributes/default.rb', :definition => 'definitions/apache_site.rb',
                          :metadata => 'metadata.rb', :provider => 'providers/site.rb', :resource => 'resources/site.rb'}[options[:file_type]]
      end
